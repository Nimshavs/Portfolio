...
contactForm.addEventListener("submit", function (e) {
  ...
  const mobileNumber = contactForm.querySelector('input[type="number"]').value;
  const emailSubject = contactForm.querySelector('input[type="text1"]').value;
  ...
  // Send email
  const mailToLink = `mailto:nimshavs@gmail.com?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(body)}`;
